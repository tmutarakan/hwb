...
async def commands_start(message: types.Message):
    ...
    await message.answer(f'{message.from_user.full_name} вас приветствует справочная.', reply_markup=kc.create_keyboard(parent='root', user_id=user_id))
...
async def file_callback_button(callback_query: types.CallbackQuery):
    ...
    user_id = callback_query.from_user.id
    await bot.answer_callback_query(callback_query.id)
    await bot.send_document(user_id, InputFile(sql_path(data)), reply_markup=kc.create_keyboard(parent=data, user_id=user_id))
    await delete_prev_markup(user_id, callback_query.message.message_id)
...
async def photo_callback_button(callback_query: types.CallbackQuery):
    ...
    user_id = callback_query.from_user.id
    await bot.answer_callback_query(callback_query.id)
    await bot.send_photo(user_id, InputFile(sql_path(data)), caption=sql_read(data), reply_markup=kc.create_keyboard(parent=data, user_id=user_id))
    await delete_prev_markup(user_id, callback_query.message.message_id)
...
async def process_callback_button(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(user_id, sql_read(data), reply_markup=kc.create_keyboard(parent=data, user_id=user_id))
    await delete_prev_markup(user_id, callback_query.message.message_id)
